...
class WeaponAttachmentTypesList(Resource):
  ...
  def get(self):
    ...
    if args['limit'] > 0:
      ...
      weaponAttachmentTypes = self.db.weaponTypes.find(query).skip(skips).limit(args['limit'])
      totalPages = round(totalElements / args['limit'])
      totalPages = totalPages
      currentPage = args['page']

      weaponAttachmentTypes = self.db.weaponAttachmentTypes.find(query).limit(args['limit'])
